                ...
                <div x-data="downloadTracker" 
                    ...
                    <template x-if="!isDownloading">
                        <form @submit.prevent="startDownload" action="{{ route('admin.episodes.downloadAll', $movie) }}" method="POST">
                            @csrf
                            <button type="submit" class="btn-primary flex items-center bg-emerald-600 border-emerald-600 hover:bg-emerald-700 hover:border-emerald-700 w-full md:w-auto" onclick="return confirm('{{ $movie->resource->name === 'Reelshort' ? 'This will mark all episodes as available for streaming. Continue?' : 'This will bulk download all un-downloaded episodes to storage in the background. Continue?' }}');">
                                ...
                                {{ $movie->resource->name === 'Reelshort' ? 'Make Available' : 'Download All' }}
                            </button>
                        </form>
                    </template>
                    <template x-if="isDownloading">
